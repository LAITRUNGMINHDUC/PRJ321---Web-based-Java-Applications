 ...
 * @author duclt
 */
public class WithdrawnServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = withdrawnPage;

        try {
            double amount = Double.parseDouble(request.getParameter("txtAmount"));
            String reason = request.getParameter("txtReason");
            boolean result = bankSessionBean.withdrawn(amount, username, reason);

            if (result) {
                request.setAttribute("NOTI", "Withdrawn OK");
            } else {
                request.setAttribute("NOTI", "Withdrawn FAILED");
            }
